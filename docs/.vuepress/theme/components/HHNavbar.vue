<template lang="pug">
  mixin nav(classes)
    nav(class=classes)
      ul.nav-links
        li(v-for="navLink of navLinks")
          router-link(:to="navLink.link" active-class="highlighted" :exact="navLink.link === '/'") {{navLink.text}}
  ...
  section.navbar
    ...
    .navbar-inner-container.padded-container
      ...
      div(@click="$emit('toggle-sidebar')").mb-show.mb-menu-toggler
        div#nav-icon3
          div
          div
          div
